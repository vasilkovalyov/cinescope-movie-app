...
export async function getTopRatedList(
...
): ResponseTVSeriesLimitedList {
  const tvSeries = await fetchGetApiTMDB<IResponseList<TVSeries>>(
    API_REQUESTS.tvTopRated,
    {
  ...
  return await Promise.all(tvSeriesIds.map((id) => getTVSeriesLimitedDetails(id, params)));
}

export async function getPopularList(params?: AppendRequestParams): ResponseTVSeriesLimitedList {
  const tvSeries = await fetchGetApiTMDB<IResponseList<TVSeries>>(
    API_REQUESTS.tvPopular,
    {
      language: DEFAULT_LANGUAGE,
      page: '1',
    },
